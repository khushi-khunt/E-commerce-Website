...
export const getApplicableCoupons = async (req, res) => {
  try {
    ...
    console.log(
      "🛒 Cart fetched:",
      cart ? JSON.stringify(cart.items, null, 2) : "Not found"
    );
    ...
    const applicable = allCoupons.filter((coupon) => {
      if (coupon.minOrderValue && cartTotal < coupon.minOrderValue) {
        console.log(
          `❌ ${coupon.code}: cartTotal ${cartTotal} < minOrderValue ${coupon.minOrderValue}`
        );
        return false;
      }
      if (coupon.maxUsage && coupon.usedCount >= coupon.maxUsage) {
        console.log(
          `❌ ${coupon.code}: usedCount ${coupon.usedCount} >= maxUsage ${coupon.maxUsage}`
        );
        return false;
      }
      ...
      if (
        ...
        !coupon.productCategory.includes("ALL")
      ) {
        const matches = productCategories.some((cat) =>
          coupon.productCategory.includes(cat)
        );
        console.log(
          `🔎 ${coupon.code}: category filter ${coupon.productCategory}. Match?`,
          matches
        );
        return matches;
      }
    ...
    const formatted = applicable.map((coupon) => ({
      ...
      description: `Get ${
        ...
        coupon.productCategory?.length &&
        !coupon.productCategory.includes("ALL")
          ? ` on ${coupon.productCategory.join(", ")}`
          : ""
