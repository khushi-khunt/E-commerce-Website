...
export const updateOrderStatusHelper = async (
  order,
  status,
  location = "Warehouse",
  note = ""
  ...
  if (order.user?.email) {
    await sendEmail(
      ...
      `Hi ${order.user.name || "Customer"},<br>Your order <b>${
        order.orderId
      }</b> has been updated to: <b>${status}</b>.<br>${
        note ? `<i>${note}</i><br>` : ""
      }Thank you!`
    );
  }
...
export const getOrderDetails = async (req, res) => {
  try {
    ...
    console.log("Fetched order detail for ID:", id);
    res.json({
      ...order,
...
export const updateOrderStatus = async (req, res) => {
  try {
    ...
    const order = await Order.findOne({orderId:id}).populate("user");
    if (!order) return res.status(404).json({ message: "Order not found" });

    if (order.status !== status) {
      await updateOrderStatusHelper(order, status);
    }
...
export const getStatusDistribution = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to get order status distribution." });
  }
};
  ...
  try {
    ...
    const flatOrders = orders.map((order) => ({
      ...
      expectedDelivery: order.expectedDeliveryDate
        ...
        : "N/A",
      products: order.items
        .map((i) => `${i.product?.title || "Unknown"} x${i.quantity}`)
        .join(","),
    }));
