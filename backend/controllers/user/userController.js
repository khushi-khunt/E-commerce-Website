...
export const updateUserProfile = async (req, res) => {
  try {
    ...
    const { name, email, password, mobile, dob, gender, city, state, country } =
      req.body;
    ...
    if (req.file && req.file.buffer) {
      ...
      console.log(
        "[UPDATE PROFILE] Uploaded new profile picture:",
        result.secure_url
      );
...
export const changeUserPassword = async (req, res) => {
  try {
    ...
    const previouslyUsed = await Promise.all(
      (user.previousPasswords || []).map((prev) =>
        bcrypt.compare(newPassword, prev)
      )
    );
...
export const setNewPasswordAfterOtp = async (req, res) => {
  try {
    ...
    if (reused.includes(true)) {
      return res.status(400).json({
        message: "You’ve already used this password. Choose a new one.",
      });
    }
