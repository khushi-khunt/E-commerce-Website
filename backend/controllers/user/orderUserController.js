...
import { updateOrderStatusHelper } from "../admin/orderAdminController.js";
...
export const checkoutFromCart = async (req, res) => {
  try {
    ...
    const stripeSession = await stripe.checkout.sessions.create({
      ...
      success_url: `${process.env.FRONTEND_URL}/user/checkout-success`,
    ...
    await sendEmail(
      ...
      `Hi ${user.name || "User"},<br/><br/>Your order <b>${generatedOrderId}</b> has been placed successfully.
        <br/>We will notify you once it is shipped.<br/><br/><b>Thank you for shopping with us!</b><br/>- khushi`,
    );
...
res.status(200).json({ message: "Items added to cart for reorder.", cart });
  } catch (error) {
